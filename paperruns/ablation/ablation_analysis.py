...
两个分析模式：
  Mode 1 (SR): 对比各算法在 Long/Short 场景下的成功率
  Mode 2 (Quality): 筛选所有算法都成功的 run_idx，对比路径长度和曲率

关键：CSV 中 Algorithm 列不区分 MHA/Duel/MD（都是 CNN-DQN/CNN-DDQN），
      因此用 variant 名 + CSV 中 Algorithm 列生成唯一算法标签。

输出：
  paperruns/ablation/results/
    sr_long.csv, sr_short.csv          — 成功率表
    quality_long.csv, quality_short.csv — 路径质量表（仅全成功 pair）
    ablation_summary.md                — Markdown 汇总报告
"""
...
RUNS = PROJ / "runs"
OUT  = PROJ / "paperruns" / "ablation" / "results"
...
# variant → 展示名（MLP 拆为两行时用 csv_algo 后缀）
VARIANT_DISPLAY = {
    "cnn_dqn":       "CNN-DQN",
    "cnn_ddqn":      "CNN-DDQN",
...
quality_tables = {}
for mode in ["sr_long", "sr_short"]:
    # 用 (variant, csv_algo) 作为唯一键，生成展示标签
    algo_dfs = {}  # label → DataFrame (indexed by run_idx)
    for v in VARIANTS:
        key = (mode, v)
        if key not in data:
            continue
    ...
    # ── Composite Score: minmax 归一化 → 加权求和（越低越好）──
...
md_lines = []
md_lines.append("# 消融实验推理结果汇总\n")
md_lines.append(f"生成日期: 2026-03-13\n")
