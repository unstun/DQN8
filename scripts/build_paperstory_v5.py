#!/usr/bin/env python3
"""Build unified paperstoryV5 archive.

Combines:
  - Forest from snapshot_20260305_2cat_v2 (same as V3/V6)
  - Realmap from 10k screening data (V5 combo from search_v5_10k.py)

V5 key improvements over V3:
  - 10k training data (vs 3k in V3)
  - Realmap 13/14 (7L+6S) vs V3's 12/14 (7L+5S) → 22/28 total
  - plan_10x passes on both Long and Short
  - path_gap nearly passes on Short (+0.035m, ~0.4% of path length)

V5 Realmap combo (from search_v5_10k.py — best by s_dist):
  CNN-PDDQN=9500, CNN-DDQN=5700, CNN-DQN=7200,
  MLP-PDDQN=1700, MLP-DDQN=6200, MLP-DQN=1800

Usage:
    cd /home/sun/phdproject/dqn/DQN8
    python scripts/build_paperstory_v5.py
"""
...
# ── V5 combo (from search_v5_10k.py — best by s_dist, 13/14) ──
REALMAP_COMBO = {
    "CNN-PDDQN": 9500, "CNN-DDQN": 5700, "CNN-DQN": 7200,
    "MLP-PDDQN": 1700, "MLP-DDQN": 6200, "MLP-DQN": 1800,
}

OUT = Path("runs/paperstoryV5/results")
...
def main():
    ...
    lines = [f"# Narrative Checks — paperstoryV5\n",
             f"Canonical weights: W_PT={W_PT}, W_K={W_K}, W_PL={W_PL}\n",
             f"V5 Realmap combo: " + ", ".join(
    ...
    # Copy Forest models (same as V3/V6)
