#!/bin/bash
# Remote test: 6 processes in parallel, 500ep, 200K buffer, no pretrain.
# Tests Python 3.10 (dqn_py310 env) to verify no segfault on 5070 Ti.
set -eo pipefail
...
for ALGO in "${ALGOS[@]}"; do
    PROFILE="test_remote_py310_${ALGO}"
...
echo ""
echo "Checkpoints:"
find "$PROJ/runs/test_remote_py310" -name "*.pt" 2>/dev/null | wc -l
