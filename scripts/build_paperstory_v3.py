#!/usr/bin/env python3
"""Build unified paperstoryV3 archive.

Combines:
  - Forest from snapshot_20260305_2cat_v2 (v1 raw CSVs)
  - Realmap from v14b screen data (V3 combo from search_v3_combo.py)

V3 key improvement over V1/V2:
  - Canonical composite weights: W_PT=1.0, W_K=0.6, W_PL=0.2
  - V3 combo found via search_v3_combo.py with correct weights
  - Realmap 12/14 (7L+5S), up from V1/V2's 11/14 (7L+4S)
...
Usage:
    ...
    python scripts/build_paperstory_v3.py
"""
...
# ── V3 combo (from search_v3_combo.py Phase 1 best, canonical weights) ──
REALMAP_COMBO = {
    "CNN-PDDQN": 2900, "CNN-DDQN": 1800, "CNN-DQN": 2200,
    "MLP-PDDQN": 700,  "MLP-DDQN": 500,  "MLP-DQN": 1400,
}

OUT = Path("runs/paperstoryV3/results")

# ── Model source for archiving ──
V14B_TRAIN = Path("runs/v14b_realmap/train_20260307_062153")
...
def main():
    ...
    lines = [f"# Narrative Checks — paperstoryV3\n",
             f"Canonical weights: W_PT={W_PT}, W_K={W_K}, W_PL={W_PL}\n",
             f"V3 Realmap combo: " + ", ".join(
    ...
    # Copy Forest models (same as V1/V2)
    ...
    # Copy Realmap models from v14b
    import shutil
    ckpt_dir = V14B_TRAIN / "checkpoints" / "realmap_a"
    for algo, ep in REALMAP_COMBO.items():
        tag = algo.lower()  # e.g. "cnn-pddqn"
