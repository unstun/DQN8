#!/bin/bash
# Screen 6 DRL algos × 10000ep (every 100ep) on Realmap, sr_long + sr_short
set -euo pipefail

PROJ="/home/sun/phdproject/dqn/DQN8"
ENV_CONDA="ros2py310"
SCREEN_DIR="$PROJ/runs/screen_6algo_10k_realmap"
SCREEN_RAW="$SCREEN_DIR/_raw"
mkdir -p "$SCREEN_RAW"

TRAIN_DIR="$PROJ/runs/algo5_10k_realmap/train_20260309_161800"
...
for EP in $(seq 100 100 10000); do
    ...
    for MODE in "${MODES[@]}"; do
        ...
        LATEST_INFER=$(cat "$PROJ/runs/screen_6algo_10k_realmap/latest.txt" 2>/dev/null || echo "")
        LATEST_DIR="$PROJ/runs/screen_6algo_10k_realmap/$LATEST_INFER"
