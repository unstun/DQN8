#!/bin/bash
# Screen 6 DRL algos × 10000ep (every 100ep) on Realmap-diag (EDT √2/2), sr_long + sr_short
set -euo pipefail

PROJ="/home/sun/phdproject/dqn/DQN8"
ENV_CONDA="ros2py310"
SCREEN_DIR="$PROJ/runs/screen_6algo_10k_realmap_diag"
SCREEN_RAW="$SCREEN_DIR/_raw"
mkdir -p "$SCREEN_RAW"

TRAIN_DIR="$PROJ/runs/algo6_10k_realmap_diag"
...
MODEL_DIR="$TRAIN_SUB/models/realmap_a"
CKPT_DIR="$TRAIN_SUB/checkpoints/realmap_a"
...
for EP in $(seq 100 100 10000); do
    ...
    for MODE in "${MODES[@]}"; do
        ...
        PROFILE="screen_6algo10k_realmap_diag_${MODE}"
        ...
        LATEST_INFER=$(cat "$PROJ/runs/screen_6algo_10k_realmap_diag/latest.txt" 2>/dev/null || echo "")
        LATEST_DIR="$PROJ/runs/screen_6algo_10k_realmap_diag/$LATEST_INFER"
